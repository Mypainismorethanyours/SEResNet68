...
class ResNet55(nn.Module):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(3, self.num_channels[0], kernel_size=3,stride=1, padding=1, bias=False)
        ...
        if self.squeeze_and_excitation: 
            self.seblock = SEBlock(channels=self.num_channels[0]) 
        ...
        self.linear = nn.Linear(self.num_channels[3]*block.expansion, num_classes) 
        """
        Dropout layer 
        """
        if self.drop: 
